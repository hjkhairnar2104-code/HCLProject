...
CATEGORIES_DATA = [
  ...
  ("15", 15, "Three Sum (3Sum)", "1. Arrays", "Sort + Two Pointers", "Medium", "Find all unique triplets summing to 0.",
   ...
   """[
     ...
     { line: 6, explanation: "i=0 (-4), l=1 (-1), r=5 (2). Sum = -3 < 0. Advance left.", vars: { sum: -3 }, visual: { type: "array_pointers", nums: [-4,-1,-1,0,1,2], ptrs: { i: 0, l: 1, r: 5 } } },
     { line: 6, explanation: "i=1 (-1), l=2 (-1), r=5 (2). Sum = 0. Found triplet [-1, -1, 2]!", vars: { match: "[-1, -1, 2]" }, visual: { type: "array_pointers", nums: [-4,-1,-1,0,1,2], ptrs: { i: 1, l: 2, r: 5 }, done: true } },
     { line: 6, explanation: "i=1 (-1), l=3 (0), r=4 (1). Sum = 0. Found triplet [-1, 0, 1]!", vars: { match: "[-1, 0, 1]" }, visual: { type: "array_pointers", nums: [-4,-1,-1,0,1,2], ptrs: { i: 1, l: 3, r: 4 }, done: true } }
   ]"""),
...
# We will generate the full file cleanly
output_js = "// LearnPath AI — 85+ Complete DSA Problem Library with Rich Multi-Step Animations & Real SVG Tree/Graph/Heap/Trie Renderers\nwindow.DSA_PROBLEMS_DATA = [\n"

entries = []
for p in CATEGORIES_DATA:
...
print("dsaProblemData.js updated successfully!")
